...
class Browscap {
   ...
   * $remoteIniUrl:   The location from which download the ini file. The
   *                  placeholder for the file should be represented by a %s.
  ...
  /**
   * The path of the local version of the browscap.ini file from which to update
   * (to be set only if used).
   *
   * @var string
   */
  ...
  public function __construct($cache_dir) {
    // has to be set to reach E_STRICT compatibility, does not affect system/app
    // settings
      ...
      if (!file_exists($cache_file) || !file_exists($ini_file) || ($interval > $this->updateInterval)) {
        try {
          ...
          if (file_exists($ini_file)) {
            ...
            // Return an array if silent mode is active and the ini db doesn't
            // exist
            return array();
          }
